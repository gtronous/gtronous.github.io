...
Process injection in memory was done for first powershell script Invoke-UrbanBishop.ps1
...
The stream is called "streamingjpegjfif" which is hinting towards it being a jfif file but the header isn't correct. So to fix this, we edit the first bytes with FF D8 FF E0. Flag is revealed.
...
```
The reverse string is flag. The intended way according to the challenge description is reversing the binary to get ahold on RSA message transmission using pcap TCP traffic.
